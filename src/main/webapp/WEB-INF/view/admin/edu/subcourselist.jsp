...
EduCoursePojo eduCoursePojo = (EduCoursePojo)request.getAttribute("eduCoursePojo");
							...
							<div class="col-sm-12">
								<div class="row">
										<div class="container-fluid">
											<div class="row">
											<div class="col-sm-10 text-center line-height-150" style="line-height: 3.5 !important;"><%=eduCoursePojo.getCourseName() %>--课程目录</div>
												<a id="add-modal" class="btn btn-info btn-lg smaller-90 bg-primary white pull-right" data-toggle="modal" data-target="" style="height:34px;">
													<span class="glyphicon glyphicon-plus"></span> 创建子课程
												</a>
												<!--table-->
												<table class="table table-bordered">													
													...
													<tbody>
													<%
													List<EduSubcoursePojo> slist = eduCoursePojo.getEduSubcourseList();
													int i=1;
													for(EduSubcoursePojo eduSubcoursePojo : slist){
													%>
														<tr>
															<td>第<%=i%>节</td>
															<td><%=eduSubcoursePojo.getSubName() %></td>
															<td><%=eduCoursePojo.getEduTeacher().getTecherName()%></td>
															<td><%=DateUtil.getDateStr(eduSubcoursePojo.getOpenTime(),"MM月dd日 HH:mm") %></td>
															<td><%=DateUtil.getDateStr(eduSubcoursePojo.getCloseTime(),"MM月dd日 HH:mm") %></td>									
															<th>
															  <%=eduSubcoursePojo.getStatus() %>														 
															</th>
															<td><%
															if(eduSubcoursePojo.getRoomId()==null && eduSubcoursePojo.getOpenTime().getTime()>System.currentTimeMillis()){
															%><button type="button" openurl="<%=path %>/course/create_room.json?subcourse_id=<%=eduSubcoursePojo.getId() %>" class="btn btn-success kaike-modal">开课</button><%} %> <button type="button" openurl="get_subcourse.htm?sid=<%=eduSubcoursePojo.getId() %>" class="btn btn-success find-modal">修改</button></td>		
														</tr>
														<%i++;} %>
			   ...
			   <div class="form-group">
			      ...
			      <div class="col-sm-5">
			         <input type="text" class="form-control" id="sub_name" name="sub_name"
			            placeholder="请输入子课名字"><input type="hidden" id="cid" name="cid" value="<%=eduCoursePojo.getId() %>">
			      </div>
			   </div>
			   <div class="form-group">
